...
PyObject *pycJSON_FileEncode(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (argtuple == NULL) {
        PyErr_SetString(PyExc_ValueError, "Failed to pack the result, required for writing to the file");
        goto fail;
    }

    PyObject *file_contents = PyObject_CallObject(write_method, argtuple);
    if (file_contents == NULL) {
        PyErr_SetString(PyExc_ValueError, "Failed to write result to the file");
        goto fail;
    }
